目录 腾讯IOT 安卓开发初探Tecent IOT 开发平台的使用新建项目创建产品添加自定义功能设备开发微信小程序配置面板配置新建设备使用设备安卓开发前置配置data.jsonapp-config.json权限配置连接平台代码安卓页面配置Activity代码微信小程序使用总结参考
...
- 设备信息：这里面是设备的一些基本属性，其中通过设备名称，设备密钥，和产品ID就可以唯一定位一个设备，然后对其进行操作。
